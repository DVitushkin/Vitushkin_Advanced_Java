...
public class Demonstrator {
    public static void main(String[] args) {
        ...
        Collection<String> paragraphA = base.stream()
        .sorted()
        .toList();

        System.out.printf("a. Collection sorted by alphabet: <%s>\n", paragraphA);

        Collection<String> paragraphB = base.stream()
        .map(item -> item + "_1")
        .toList();

        System.out.printf("b. To each elem added <_1>: <%s>\n", paragraphB);

        Object[] paragraphC = base.stream()
        .map(item -> Integer.parseInt(String.valueOf(Character.getNumericValue(item.charAt(1)))))
        .toArray();
        ...
        Collection<String> paragraphD = base.stream()
        .sorted()
        .distinct()
        .toList();
